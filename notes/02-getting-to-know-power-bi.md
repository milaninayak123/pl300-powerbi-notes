...
The regional manager, though, doesn't want to dig through five pages every morning. So the analyst pins the key numbers like total orders, average delivery time, revenue etc onto a single-page **Dashboard**. One glance tells the manager if something's wrong; clicking a tile takes them into the full report for detail.
